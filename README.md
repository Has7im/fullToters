# Full Toters
Special application in food other services



## Badges


![MIT License](https://img.shields.io/badge/Dart-2.2-orange)
![GPLv3 License](https://img.shields.io/badge/Node.js-16.17-blue)
![AGPL License](https://img.shields.io/badge/Vs%20Code-1.71.2-red)
![AGPL License](https://img.shields.io/badge/Android%20Studio-2021.2-green)
![AGPL License](https://img.shields.io/badge/Flutter-3.0.5-lightgrey)


![Logo](data:image/png;base64,)


## pub.dev - packeges



| packege - name | Ver     | Description                |
| :-------- | :------- | :------------------------- |
| `bottom_navy_bar` | `6.0.0` | **Required**.A beautiful and animated bottom navigation. The navigation bar use your current theme, but you are free to customize it |
| `http` | `0.13.5` | **Required**.The easiest way to use this library is via the top-level functions. They allow you to make individual HTTP requests with minimal hassle: |
| `intl` | `0.17.0` | **Required**.Provides internationalization and localization facilities, including message translation, plurals and genders, date/number formatting and parsing, and bidirectional text. |




## Screenshots

![App Screenshot](https://a.top4top.io/p_24538t9055.jpg)
![App Screenshot](https://h.top4top.io/p_2453el5gw1.jpg)
![App Screenshot](https://j.top4top.io/p_2453pteiw2.jpg)
![App Screenshot](https://k.top4top.io/p_2453wq9f83.jpg)
![App Screenshot](https://l.top4top.io/p_2453th8nf4.jpg)


## Installation


```bash
  1-Download project
```
```bash
  2-unzip project
```
```bash
  3-Open Android Stadio 
```

## Deployment

First time run this commned in terminal

```bash
  flutter pub get
```


## Authors

- [@Hashim](https://github.com/Has7im)


## License

[Dart](https://dart.dev/)

[flutter](https://flutter.dev/)

[Node.js](https://nodejs.org/en/)

